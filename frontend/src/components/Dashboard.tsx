...
export default function Dashboard({ userId, userNameSession }: DashboardProps) {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const [contas, transacoes, bancosFiltro, cartoes] = await Promise.all([
      ...
      const listaCartoes = Array.isArray(cartoes) ? cartoes : [];
  ...
  const handleEditClick = (tx: any) => {
    ...
    setTxCreditCardId(tx.creditCardId ? tx.creditCardId.toString() : '');
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 animate-fade-in">
      ...
      <div className="rounded-xl border-l-4 border-success-500 bg-white p-6 shadow-card transition-shadow duration-200 hover:shadow-card-hover">
        <h2 className="text-xs font-semibold uppercase tracking-wider text-neutral-500 sm:text-sm">Saldo Consolidado (Patrimônio)</h2>
            ...
            <form onSubmit={handleSaveTransaction} className="space-y-3">
              ...
              <div className="flex gap-2">
                ...
                <select 
                  ...
                  disabled={!!txCreditCardId}
                  aria-label="Tipo da transação" 
                  className={`w-1/3 rounded-lg border border-neutral-300 p-2.5 text-sm font-semibold outline-none transition-colors duration-200 focus:ring-2 focus:ring-brand-500/30 ${txType === 'INCOME' ? 'text-success-600' : 'text-danger-600'} ${txCreditCardId ? 'bg-neutral-100 cursor-not-allowed opacity-70' : 'bg-white'}`}
                >
              ...
              <select 
                ...
                onChange={e => {
                  setTxCreditCardId(e.target.value);
                  if (e.target.value) setTxType('EXPENSE'); 
                }} 
          ...
          <div className="rounded-xl border border-neutral-200 bg-white p-5 shadow-card">
            ...
            <form onSubmit={handleCreateAccount} className="space-y-3">
              <div>
                ...
                <select id="dash-bank" required value={selectedBankName} onChange={e => setSelectedBankName(e.target.value)} className="w-full rounded-lg border border-neutral-300 p-2.5 text-sm text-neutral-900 outline-none transition-colors duration-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/30">
              ...
              <div>
                ...
                <select id="dash-acctype" value={accountType} onChange={e => setAccountType(e.target.value)} className="w-full rounded-lg border border-neutral-300 p-2.5 text-sm text-neutral-900 outline-none transition-colors duration-200 focus:border-brand-500 focus:ring-2 focus:ring-brand-500/30">
                  <option value="CORRENTE">Conta Corrente</option>
                  <option value="POUPANCA">Poupança</option>
                  <option value="CARTEIRA">Carteira / Dinheiro Vivo</option>
                </select>
              </div>
              <button type="submit" className="w-full rounded-lg bg-neutral-800 py-2.5 text-xs font-semibold text-white transition-colors duration-200 hover:bg-neutral-900 focus:outline-none focus-visible:ring-2 focus-visible:ring-neutral-700 focus-visible:ring-offset-2">+ Adicionar Conta Oficial</button>
            </form>
          </div>
      ...
      <CreditCardManager cards={creditCards} onUpdate={loadDashboardData} />
